...
class ConfigurationClassEnhancer {
	...
	private static final Callback[] CALLBACKS = new Callback[] {
			//增强方法，控制bean的作用域
			//不让其每次都是new，而是第一次new，后面的都从容器中获取
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		...
		/**
		 * 如果没有被代理，就进行CGLIB代理，创建一个enhance对象
		 */
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		//主要是为了生成的CGLIB类中添加成员变量$$beanFactory
		//同时实现接口EnhancedConfiguration，EnhancedConfiguration的父接口BeanFactoryAware中有setBeanFactory方法
		//设置此变量的值为当前Context的BeanFactory，这样一来我们这个CGLIB代理对象就有了BeanFactory
		//有了BeanFactory就能获得对象，而不用通过方法获得对象了，因为通过方法获得对象不能控制其过程
		//该BeanFactory的作用就是在this调用时拦截该调用，并直接在BeanFactory中获得目标bean
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));
		/**
		 * CALLBACK_FILTER：拦截器链;相当于多个回调方法
		 */
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			//通过enhancedConfigInstance中的cglib生成的队员变量$$beanFactory获得beanFactory
			...
			//判断执行的方法与调用的方法是不是同一个方法（判断到底是new还是get）
