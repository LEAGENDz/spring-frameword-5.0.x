...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	/**
	 * 同样顾名思义，这个是一个扫描器，扫描所有加了注解的Bean
	 * 同样是在构造方法中实例化
	 * 可以用来扫描包或者类，继而转换成BeanDefinition
	 * 但是实际上我们扫描包工作不是scanner这个对象完成的
	 * 而是spring自己new的一个ClassPathBeanDefinitionScanner完成的
	 * 这里的scanner仅仅是为了程序员能够在外部调用AnnotationConfigApplicationContext对象时scan方法
	 * annotationConfigApplicationContext.scan("xxxx")
	 */
	 ...
	 * 然后会把这个被注解了的JavaConfig的类通过注解读取器读取后继而解析
	 */
